...
async function findById(id) {
    const { rows } = await db.query(
        'SELECT id, user_id, balance, description, created_at, created_by, updated_at, updated_by FROM banks WHERE id = $1',
        [id]
    )
...
async function create(clientOrData, maybeData){
    ...
    const query = `INSERT INTO banks (user_id, balance, description, created_by, updated_by) VALUES ($1, $2, $3, $4, $5) RETURNING id, user_id, balance, description, created_at, created_by, updated_at, updated_by`
