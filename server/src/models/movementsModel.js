...
async function findById(id){
    const sql = `SELECT id, transactions_id, month_movement, created_at, created_by, updated_at, updated_by FROM movements_calculations WHERE id = $1`
    const { rows } = await db.query(sql, [id])
    return MapRowToPublic(rows[0] || null)
}

async function find(){
    const sql = `SELECT id, transactions_id, month_movement, created_at, created_by, updated_at, updated_by FROM movements_calculations`
...
async function create(clientOrData, maybeData) {
    ...
    const result = maybeData? await client.query(sql, params) : await db.query(sql, params)
